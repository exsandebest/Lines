...
int currentScore;
QString colors[] = {"blue","green","red","purple","orange"};
int next3[3];
int field[9][9];
int active[3] = {-1};
int cost = 20;
    ...
    for (int i = 0; i < 9; ++i){
        for (int j = 0; j < 9; ++j){
            ...
            if (a == -1){
                ...
                btn->setIcon(QIcon(":/src/img/"+colors[a]+".png"));
...
void MainWindow::btnGameClicked(){
    ...
    if(color == "no" && checkAccess(x1,y1,x2,y2)){
        ...
        while (1){
           ...
           QPushButton * btn = new QPushButton();
           btn->setIcon(QIcon(":/src/img/"+colors[active[2]]+".png"));
           btn->setIconSize(QSize(90,90));
           QString s1,s2;
           s1.setNum(x2);
           s2.setNum(y2);
        ...
        btn3->setIcon(QIcon(":/src/img/"+colors[active[2]]+".png"));
    ...
    QStringList list = btn->property("id").toString().split(":");
        ...
        btn2->setFixedSize(100,100);
        btn2->setObjectName("btnGame");
        btn2->setIcon(QIcon(":/src/img/"+colors[active[2]]+".png"));
...
while (k<3){
    ...
    if (color == "no"){
        ...
        btn->setIcon(QIcon(":/src/img/"+colors[c]+".png"));
        btn->setStyleSheet("border-radius: 50px;border: 6px solid white;");
        btn->setIconSize(QSize(90,90));
...
for (int i = 0; i < 3; ++i){
QPushButton * btn = new QPushButton();
btn->setIcon(QIcon(":/src/img/"+colors[next3[i]]+".png"));
btn->setIconSize(QSize(60,60));
btn->setObjectName("btnGame");
btn->setStyleSheet("border-radius: 35px;border: 6px solid white;");
btn->setFixedSize(70,70);
